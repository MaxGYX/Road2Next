...
### 关于生成效果的几个思考
-   训练需要的算力是很大的，于是VAE和diffusion采用了预训练的模型
-   音乐特征子空间提取的网络也是用了预训练模型，而音乐频谱数据的特征应该有自己独特的地方，卷积核可能需要精心设计来完成各类子空间特征的提取。
-   LSTM网络提取音乐注意力特征向量hA，这部分也应该精心设计，最好是应该有监督的数据，比如找一些人听音乐来对音乐对人注意力的影响做多维度的评判，得到监督标签。目前直接将频谱图经过lstm与其他模型一起做loss的方式，只依赖最终生成音乐的结果，实际并不会太好。
-   音乐貌似不应该是大力出奇迹，需要有把旋律，节奏数据从图像特征角度来评估的方法。（从这点来说，貌似回到了之前对midi判断的老路？？）
